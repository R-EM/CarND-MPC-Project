			...
			if (s != "") 
			{
				...
				if (event == "telemetry") 
				{
					...
					for (int i = 0; i < ptsx.size(); i++)
					{
						...
						ptsx_new.push_back(shift_x*cos(-psi) - shift_y*sin(-psi));
						ptsy_new.push_back(shift_x*sin(-psi) + shift_y*cos(-psi));
					}
					...
					// double epse = psi - atan(coeffs[1] + 2 * px * coeffs[2] + 3*coeffs[3] * pow(px,2))
